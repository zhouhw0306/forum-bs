...
public class SubScribeController {
    ...
    public Result<Void> addFollow(String authorId) {
        ...
        boolean flag = subscribeService.save(Subscribe.builder().beSubscribe(authorId).subscribe(currentUser).build());
        if (flag) {
            // 触发关注通知
            try {
                Notification notify = new Notification();
                notify.setType(NotificationTypeEnum.FOLLOW.getCode());
                notify.setTitle("新关注");
                notify.setContent("有用户关注了你");
                notify.setSenderId(currentUser);
                notify.setReceiverId(authorId);
